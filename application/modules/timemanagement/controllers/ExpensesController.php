	...
	public function editAction()
	{
		...
		if(sizeof($clientsData) > 0){
			foreach ($clientsData as $client){
				$expensesForm->esic_no->addMultiOption($client['id'],$client['client_name']);
			}
		}else{
		...
		try
		{
			...
			{	//Edit Record...
				if(is_numeric($id) && $id>0)
				{
					...
					$projectsData = $projectsModel->getProjectListByClientID($data[0]['esic_no']);
		...
		if($this->getRequest()->getPost()){
			if($expensesForm->isValid($this->_request->getPost())){
				...
				$clientID = $this->_request->getParam('esic_no');
