...
class Default_Model_Empjobhistory extends Zend_Db_Table_Abstract
{
	...
	public function getEmpJobHistoryData($sort, $by, $pageNo, $perPage,$searchQuery,$id)
	{
		...
		$empjobhistoryData = $this->select()
						   ...
						   ->joinLeft(array('c'=>'tm_clients'),'c.id=e.esic_no AND c.is_active = 1',array('esic_no'=>'c.client_name'))
						   ->where($where)
    					   ->order("$by $sort") 
	...
	{		
		...
		$tableFields = array('action'=>'Action','department'=>'Previous Employe','ESIC No.'=>'Client','start_date'=>'From','end_date'=>'To');
