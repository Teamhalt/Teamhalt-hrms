	...
	public function editpopupAction()
	{
		...
		if(sizeof($clientData) > 0)
		{
			foreach ($clientData as $client){
				$projectsForm->esic_no->addMultiOption($client['id'],$client['client_name']);
			}

		}else
		{
			$msgarray['esic_no'] = 'Clients are not configured yet.';
			$emptyFlag++;
		}
	   ...
	   $projectsForm->setAction(BASE_URL.'projects/editpopup/id/'.$id.'/unitId/'.$unitid);
		...
		try
		{
			...
			{	//Edit Record...
				...
				if($this->getRequest()->getParam('cid')){
					...
					$projectsForm->esic_no->setValue($cid);
				}
				$this->view->form = $projectsForm;
				$this->view->ermsg = '';
				$this->view->id = '';
		...
		if($this->getRequest()->getPost()){
			if($projectsForm->isValid($this->_request->getPost())){
				...
				$esic_no = $this->_request->getParam('esic_no');
			...
			}else
			{
				...
				$msgarray['esic_no'] = 'Clients not configured yet.';
