	...
	public function viewpopupAction()
	{
		...
		if($id)
		{
			...
			if(!empty($data))
			{
				...
				$clientsArr = array();
				if(is_numeric($data[0]['esic_no']))
				{				
					$clientsArr = $clientsModel->getClientDetailsById($data[0]['esic_no']);
				}
				if(!empty($clientsArr))
				{
					$empjobhistoryform->client->addMultiOption($clientsArr[0]['id'],$clientsArr[0]['client_name']);
					$data[0]['esic_no']=$clientsArr[0]['client_name'];
				}
                else
				{
					$data[0]['esic_no']=  "";
	...
	public function editpopupAction()
	{
		...
		if($id)
		{
			...
			if(!empty($data))
			{
				...
				$empjobhistoryform->setDefault('client',$data[0]['esic_no']);
				if(isset($data[0]['start_date']) && $data[0]['start_date'] !='')
				{
					$start_date = sapp_Global::change_date($data[0]['start_date'], 'view');
					$empjobhistoryform->start_date->setValue($start_date);
