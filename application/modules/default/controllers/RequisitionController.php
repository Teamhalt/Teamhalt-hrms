	...
	public function addAction()
	{
		...
		if(sizeof($clientData) > 0)
		{
			foreach ($clientData as $client){
				$form->esic_no->addMultiOption($client['id'],$client['client_name']);
			}
		}
		else
		{
			$norec_arr['esic_no'] = 'Clients are not configured yet.';
		}
	...
	public function save($requisitionform,$data)
	{
		...
		if($requisitionform->isValid($this->_request->getPost()))
		{
			...
			try
			{
                ...
                $esic_no= $this->_getParam('esic_no',null);
        ...
        {                        
            ...
            if(is_numeric($id) && $id >0)
            {
                ...
                if(count($data)>0  && $data[0]['req_status'] == 'Initiated')
                {
					...
					if($data['esic_no'] != '')
					{
						$clien_data = $clientsModel->getClientDetailsById($data['esic_no']);
					    $data['esic_no']=$clien_data[0]['client_name'];
					}  
					if($data['recruiters'] != '')
					{
						$name = '';
